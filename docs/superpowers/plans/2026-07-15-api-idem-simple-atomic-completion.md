...
- Access/authz resolvers (`requireTripAccess`, `assertTripWrite`, `resolveDayWrite`, `resolvePlaceWrite`, `resolveNoteForItem`) ARE threaded with `exec` too (revised during execution — the original "keep authz on module db" was not integration-testable, since module `db` never connects under vitest). `getTripRole`/`loadTripBasic` are `cache()`-wrapped, so authz threading uses a new **uncached** `getTripRoleWith(tripId, userId, exec)` (with `getTripRole` kept as the cached wrapper for existing callers) and an inline trip-existence probe on `exec` inside `requireTripAccess`. Net effect: the whole mutation — authz reads + writes + completion — runs on one executor (consistency win, no runtime downside). `writeAudit` remains the only deliberate module-`db` exception.
